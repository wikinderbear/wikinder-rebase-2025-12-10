...
[^1]: Three blocks have a middle one; four don't. However, if three blocks form a triangle, we may not know which one is the middle. A set with an odd number of elements has a middle element if the elements are arranged in a line. It seems that such an ordering is said to be a _linear order_ (or _total order_).
